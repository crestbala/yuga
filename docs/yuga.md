...
|---|---|---|
| Load | `src/compile.c` | Resolve `import "std:foo"` → `std/foo.yuga`, relative `"path.yuga"` from the importer. Cycles are errors. |
...
yuga/
  ...
  zeus/           kit + hosts: desktop/ Cocoa, ios/ UIKit, android/ Canvas, web/ Canvas2D
  tests/          compile_pass / compile_fail / examples
  zeus/ widget library + kit apps  (see zeus.md)
...
./bin/yugac --target=ios --run zeus/apps/dashboard/dashboard.yuga  # Simulator
./bin/yugac --target=android zeus/examples/counter/android/app.yuga  # Gradle project
```
...
- Runtime files (`zeus_plat.c`, `zeus_key.c`, `zeus/desktop/mac.m`) compile once into
  `runtime/.obj/` and are reused until those sources change.
...
The module name is the file stem (`math`), or `foo` from `std:foo`.
...
| A tiny RPC server | `import "std:http"` (`tests/examples/http_server.yuga`) |
| A 3D/2D toy scene | `import "std:maya"` (`tests/examples/solar.yuga`) |

Corpus you can compile as examples:

- `tests/compile_pass/hello.yuga`, `vec.yuga`, `globals.yuga`, `import_math.yuga`
- `tests/examples/counter.yuga`, `fib.yuga`, `http_server.yuga`
- Failures the checker must reject: `tests/compile_fail/*.yuga`
