...
```
ZEUS_HEADLESS=1 ./bin/yugac tests/compile_pass/zeus_snap.yuga -o tests/tmp/zeus_snap
```
...
```
./bin/yugac build --target=wasm32 zeus/examples/counter/frontend/app.yuga
cd zeus/examples/counter/frontend && npm install && npm run dev
...
```
./zeus/examples/counter/ios/run.sh
```
...
```
./zeus/examples/counter/backend/run.sh
./bin/yugac --target=ios --run zeus/examples/counter/ios/app.yuga
```

Needs Xcode and an iPhone Simulator. Output is
`zeus/examples/counter/ios/build/app.app`.
...
Step-by-step: `zeus/examples/counter/android/guide.md`. On macOS with Homebrew:
...
Output is the Gradle tree `zeus/examples/counter/android/build/app`.
...
```
./zeus/examples/counter/macos/run.sh
```
...
```
./zeus/examples/counter/backend/run.sh
./bin/yugac --target=native --run zeus/examples/counter/macos/app.yuga
```

Output is `zeus/examples/counter/macos/build/app`.
...
zeus/lib/                  kit widgets (shared)
zeus/apps/                 same apps on every host
zeus/desktop/mac.m         Cocoa present
zeus/ios/ios.m             iOS Simulator present (paint only)
zeus/android/android.c     JNI + Android Canvas present (paint only)
zeus/web/loader.js         Canvas2D host
zeus/web/wasm.c            WASM entry / JS imports
runtime/zeus_plat.c        shared C seam
runtime/zeus_key.c         keyboard
runtime/net.c              TCP trampolines (`std/http` is Yuga)
zeus/examples/counter/frontend   wasm UI (`http.client("").call`)
zeus/examples/counter/macos      Cocoa UI (`http.client(api.native_addr())`)
zeus/examples/counter/ios        Simulator UI (`http.client(api.native_addr())`)
zeus/examples/counter/android    emulator UI (`http.client(api.android_addr())`)
zeus/examples/counter/screen.yuga  shared page (signals + kit widgets)
zeus/examples/counter/backend    shared `api.yuga` + native `server.yuga`
```
